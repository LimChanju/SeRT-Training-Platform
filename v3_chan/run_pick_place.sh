...
export HRI_COLLECTION_LABEL="${HRI_COLLECTION_LABEL:-surface_twist_v8_dualclock_tracked_v1}"
...
export HRI_COLLECTION_TEST_MODE="${HRI_COLLECTION_TEST_MODE:-0}"
...
export BHAPTICS_ENABLED="${BHAPTICS_ENABLED:-1}"
case "${BHAPTICS_ENABLED,,}" in
    0|false|no|off)
        export BHAPTICS_NOTEBOOK_IP=""
        ;;
    *)
        export BHAPTICS_NOTEBOOK_IP="${BHAPTICS_NOTEBOOK_IP:-10.3.129.185}"
        ;;
esac
export BHAPTICS_UDP_PORT="${BHAPTICS_UDP_PORT:-5005}"
export BHAPTICS_INTENSITY="${BHAPTICS_INTENSITY:-100}"
export BHAPTICS_MIN_INTERVAL="${BHAPTICS_MIN_INTERVAL:-0.08}"
...
export HRI_TRAJECTORY_PATH="$PROJECT_ROOT/v3_chan/trajectories/hri_vr_sphere_${HRI_COLLECTION_LABEL}_${HRI_SESSION_ID}.hdf5"
...
if [[ "$HRI_PRODUCTION_MODE" == "1" && "${HRI_ENFORCE_EXPERIMENT_METADATA:-1}" != "0" ]]; then
    for metadata_key in \
        ...
        if [[ -z "$metadata_value" || "${metadata_value,,}" == "unspecified" ]]; then
            printf '[Collect] missing required experiment metadata: %s\n' "$metadata_key" >&2
            printf '[Collect] set metadata variables or use HRI_COLLECTION_TEST_MODE=1 for a non-study test.\n' >&2
            exit 2
        fi
    done
...
printf '[Collect] participant_session=%s handedness=%s practice=%s\n' \
    "$HRI_PARTICIPANT_SESSION_INDEX" "$HRI_PARTICIPANT_HANDEDNESS" "$HRI_IS_PRACTICE"
printf '[Collect] condition=%s block=%s haptic_condition=%s\n' \
    "$HRI_EXPERIMENT_CONDITION" "$HRI_EXPERIMENT_BLOCK_ID" "$HRI_HAPTIC_CONDITION"
...
if grep -Fq '[HRI] collection complete:' "$LOG_PATH"; then
    ...
    if [[ "$HRI_PRODUCTION_MODE" == "1" ]]; then
        ...
        if "$_validator_python" "$PROJECT_ROOT/v3_chan/validate_hri_collection.py" \
            ...
            printf '[Collect] validator failed; speed order counter will not advance.\n' >&2
        fi
    else
        _collection_valid=1
    fi
...
if [[ "$_speed_order_auto" == "1" ]]; then
    if [[ "$_collection_complete" == "1" && "$_collection_valid" == "1" ]]; then
